...
from bibalabel.bladerTools import *
import copy
from bibalabel.statisticalMeasurements import *
...
class PredictionEvaluation():
    ...
    def maskOff(self,event):

        color = self.clr2
        self.clr2 = self.clr
        self.clr = color
        self.clrC = abs(1 - self.clrC)
        self.clrEntry['text'] = 'XX' if self.clrC else '    '
        self.imgtk = readOverlay(self.newImageList[self.count][0], self.clr)
        self.canvas.itemconfig(self.image_on_canvas,
                               image=self.imgtk)
